...
function drop(event){
	...
	let validStep = countSteps(noOfStepsCount, chessArray[p_i][p_j].color);
	if(check && validStep){
		updateInLocalStorage(p_i,p_j,n_i,n_j,chessArray);
		chessArray = JSON.parse(localStorage.getItem('chess-board'));
		let kingBlack = checkPosition('chess_king','black',chessArray);
		kingInThreat(kingBlack, kingBlack.row_pos, kingBlack.col_pos,chessArray);
